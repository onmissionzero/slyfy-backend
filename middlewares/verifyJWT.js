            ...
            if (error.name === 'TokenExpiredError') {
                try {
                    ...
                    jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, async (err, decodedRefresh) => {
                        ...
                        res.cookie('jwt_access_token', newAccessToken, { httpOnly: true, sameSite: 'None', secure: true, maxAge: 60 * 60 * 1000 });
                        res.cookie('jwt_refresh_token', newRefreshTokenRefreshToken, { httpOnly: true, sameSite: 'None', secure: true, maxAge: 3 * 24 * 60 * 60 * 1000 });
